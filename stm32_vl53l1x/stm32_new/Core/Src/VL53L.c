...
void VL53L_Process_PLC_Response(const uint8_t *buf, uint16_t len) {
    ...
    if (p_len >= 12) {
        ...
        // Byte 12-13: Word 0 = D910 (Target mm: ví dụ 300)
        // Byte 14-15: Word 1 = D911 (Mã lệnh Calib: 10, 20, 30)
        if (p_len >= 16) {
            uint16_t plc_d910 = payload[12] | (payload[13] << 8); // D910 = Word 0 (Byte 12-13)
            uint16_t plc_d911 = payload[14] | (payload[15] << 8); // D911 = Word 1 (Byte 14-15)
